...
@section('content')
<div class="space-y-6">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5" data-aos="fade-up" data-aos-delay="80">
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <p class="text-sm text-gray-500">إجمالي الإيرادات</p>
            <p class="text-3xl font-extrabold text-green-600 mt-1"><span data-counter data-value="{{ $totalRevenue }}" data-decimals="2">0</span> ر.س</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <p class="text-sm text-gray-500">إيرادات هذا الشهر</p>
            <p class="text-3xl font-extrabold text-blue-600 mt-1"><span data-counter data-value="{{ $monthlyRevenue }}" data-decimals="2">0</span> ر.س</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <p class="text-sm text-gray-500">مبالغ معلقة</p>
            <p class="text-3xl font-extrabold text-red-600 mt-1"><span data-counter data-value="{{ $pendingAmount }}" data-decimals="2">0</span> ر.س</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6" data-aos="fade-up" data-aos-delay="200">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            @forelse($paymentMethods as $method)
                ...
                @endphp
                <div class="p-5 bg-gray-50 rounded-xl text-center" data-aos="fade-up" data-aos-delay="{{ 180 + $loop->index * 40 }}">
                    <i data-lucide="{{ $icons[$method->payment_method] ?? 'receipt' }}" class="w-8 h-8 mx-auto text-green-600 mb-2"></i>
                    <p class="font-bold text-gray-800 text-lg" data-counter data-value="{{ round($method->total) }}">0</p>
